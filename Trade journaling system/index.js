...
function generateReport() {
  ...
  // === Calculate per-strategy win rates ===
  const strategies = {};

  trades.forEach(t => {
    const strategy = t.note.toLowerCase().trim();
    if (!strategies[strategy]) {
      strategies[strategy] = { wins: 0, losses: 0, total: 0 };
    }
    strategies[strategy].total++;
    if (t.amount > 0) strategies[strategy].wins++;
    if (t.amount < 0) strategies[strategy].losses++;
  });

  let strategyReport = '<h4>📈 Strategy-wise Win Rates:</h4><ul>';
  for (const [name, data] of Object.entries(strategies)) {
    const rate = (data.wins / data.total) * 100 || 0;
    strategyReport += `<li><strong>${name}</strong>: ${rate.toFixed(2)}% win rate (${data.wins}W / ${data.losses}L)</li>`;
  }
  strategyReport += '</ul>';
